...
private fun DraggableSettingsButton(
    ...
    BoxWithConstraints(modifier) {
        ...
        var localX by remember(maxXPx) { mutableFloatStateOf(x.coerceIn(0f, 1f) * maxXPx) }
        var localY by remember(maxYPx) { mutableFloatStateOf(y.coerceIn(0f, 1f) * maxYPx) }
        var dragging by remember { mutableStateOf(false) }
        var draggedSinceLastClick by remember { mutableStateOf(false) }

        LaunchedEffect(x, y, maxXPx, maxYPx, dragging) {
            if (!dragging) {
                localX = x.coerceIn(0f, 1f) * maxXPx
                localY = y.coerceIn(0f, 1f) * maxYPx
            }
        }

        androidx.compose.material3.SmallFloatingActionButton(
            onClick = {
                if (draggedSinceLastClick) draggedSinceLastClick = false else onOpen()
            },
            modifier = Modifier
                .graphicsLayer {
                    translationX = localX
                ...
                .pointerInput(maxXPx, maxYPx) {
                    detectDragGestures(
                        onDragStart = { dragging = true; draggedSinceLastClick = false },
                        ...
                        onDrag = { change, dragAmount ->
                            change.consume()
                            if (dragAmount.getDistance() > 0.5f) draggedSinceLastClick = true
